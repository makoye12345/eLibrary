...
<div class="container mx-auto p-4">
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category->id) }}" class="max-w-md">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <select name="category_id" class="w-full border p-2 rounded @error('category_id') border-red-500 @enderror">
                <option value="" {{ old('category_id', $category->category_id) === null ? 'selected' : '' }}>None</option>
                @foreach (\App\Models\Category::whereNull('category_id')->where('id', '!=', $category->id)->orderBy('name')->get() as $parent)
                    <option value="{{ $parent->id }}" {{ old('category_id', $category->category_id) == $parent->id ? 'selected' : '' }}>
                        {{ $parent->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" class="w-full border p-2 rounded @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
